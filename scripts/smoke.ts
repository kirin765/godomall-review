...
const withImg = reviews.filter((r) => r.images.length > 0);
const withoutImg = reviews.filter((r) => r.images.length === 0);
log(withImg.length === 5, `이미지 포함 5건 (실제 ${withImg.length})`);
log(withoutImg.length === 1, `이미지 없는 1건 (실제 ${withoutImg.length})`);
log(withImg.every((r) => r.images.every((u) => /^https?:\/\//.test(u))), '이미지 URL 형식 유효');
