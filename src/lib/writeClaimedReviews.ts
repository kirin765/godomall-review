export type WriteOutcome = {
  ...
  uncertainCharged: number; photoDropped: number; retryableIndices: number[]; failMessage: string[];
};
...
export async function writeClaimedReviews<Row>(options: {
  ...
  for (let start = 0; start < rows.length; start += options.batchSize) {
    const chunk = rows.slice(start, start + options.batchSize);
    ...
    if (outcome === 'rejected') { result.failed += chunk.length; result.permanentFailed += chunk.length; }
    if (outcome === 'retryable') {
      retryable(start, rows.length - start);
      result.failMessage.push(message || '잠시 후 다시 시도해 주세요.'); break;
    }
    if (message) result.failMessage.push(message);
    if (options.delayMs && start + chunk.length < rows.length) await new Promise(resolve => setTimeout(resolve, Math.min(options.delayMs!, Math.max(0, deadline - Date.now()))));
  }
  return result;
}
