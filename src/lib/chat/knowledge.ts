...
const PLATFORM = {
  ...
  /** 현재 앱 상태 — 고객에게 안내할 문구 */
  status:
    '유료 기능은 준비 중입니다. 결제(PG) 연동·심사가 진행 중이라 아직 결제를 받지 않고, ' +
    '지금은 무료 20건까지 리뷰를 옮길 수 있습니다. 유료 오픈 전까지는 무료 한도 안에서 이용해 주세요.',
  priceNote: '월 9,900원',
};
...
const ERROR_GUIDES: { when: string; guide: string }[] = [
  {
    ...
    guide:
      `지금은 무료 ${PLATFORM.freeQuota}건까지 옮길 수 있고, 모두 사용하면 더 이상 등록되지 않습니다. ` +
      `유료 결제는 준비 중(결제 PG 연동·심사 진행 중)이라 아직 넘어갈 수 없습니다. ` +
      `다음 달 리뷰를 옮기고 싶다면 ${SUPPORT.supportEmail}로 문의해 주세요.`,
  },
  {
  ...
  },
  {
    ...
    guide:
      `${SUPPORT.appName}은 유료 전환을 준비 중입니다. 결제(PG) 연동·심사가 진행 중이라 아직 결제를 받지 않으며, ` +
      `준비되면 공지하겠습니다. 지금은 무료 ${PLATFORM.freeQuota}건까지 이용할 수 있고, 자세한 일정은 ${SUPPORT.supportEmail}로 문의해 주세요.`,
  },
  {
